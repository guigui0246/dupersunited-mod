...
public class PluginScanner {
    ...
    public static void startScan() {
        ...
        foundPlugins.clear();
        scanning = true;
        completionId = ((IClientCommandSource) mc.getNetworkHandler().getCommandSource()).dupersunited$beginCompletion();
        ticksWaiting = 0;
    ...
    public static void onCommandSuggestions(CommandSuggestionsS2CPacket packet) {
        if (scanning && packet.id() == completionId) {
            ...
            ((IClientCommandSource) mc.getNetworkHandler().getCommandSource()).dupersunited$endCompletion();
